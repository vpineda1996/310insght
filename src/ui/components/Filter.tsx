...
export class RoomFilter extends React.Component<RoomFilterProps, RoomFilterState> {
    ...
    renderCheckBox = () => {
        return this.state.checkboxOpions.filter(opt =>
            !this.props.subfilters ||
                !this.props.subfilters[opt] ||
                !this.props.value.includes(opt)).map((opt: any) => (
                        ...
                        value={key}>{key}</button>
                        {
                            ...
                            <div key={'checkbox-subfilter-'+this.props.field+'-'+key+'-'+opt}>
                                <input
                                    type='checkbox'
                                    value={key+'-----'+opt+'-----'+dep}
                                    id={'checkbox-subfilter-'+this.props.field+'-'+key+'-'+opt+'-box'}
                                    onChange={this.onSelect}
                                    checked={this.props.subfilters &&
                                        this.props.subfilters[key] &&
                                        this.props.subfilters[key][opt][dep]} />
    ...
    onSelect = (e: any) => {
        ...
        if (this.state.dependencies && e.target.value.split('-----').length === 1 && !this.state.dependencies[e.target.value]) {
    ...
    fetchSubOptions = (source: string) => {
        ...
        Store.fetch('room-suboptions', {
            ...
            let conversionMap: {[opt:string]: string} = {};
            let initialValues: { [opt: string]: {[dep:string]: boolean}} = {};
            cols.forEach(col => {
            ...
            data.forEach(val => {
                Object.keys(val).forEach(key => {
                    ...
                    if (!initialValues[val[key]]) { initialValues[val[key]] = {} }
                    initialValues[val[key]][conversionMap[key]] = true;
                });
            });
            this.setState(state);

            this.props.onSelect(this.props.field, source+'-----new', initialValues);
        });
    }
